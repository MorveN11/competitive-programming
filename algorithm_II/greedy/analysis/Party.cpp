...
int getMax(vi minors, vi majors, int Nminors, int Nmajors)
{
  int ans = 0;
  rep(i, 0, Nminors)
  {
    if (i < Nmajors)
    {
      ans += majors[i] - minors[i];
    }
    if ((i + 1) < Nminors)
    {
      ans += majors[i] - minors[i + 1];
    }
    if (i >= Nmajors || (i + 1) >= Nminors)
    {
      break;
    }
  }
...
void solve()
{
  ...
  sortt(nms);
  vi minors(Nminors), majors(Nmajors);
  rep(i, 0, Nminors)
  {
    minors[i] = nms[i];
  }
  ...
  coutt(getMax(minors, majors, Nminors, Nmajors));
}
