...
void knapsack()
{
  for (int i = 1; i <= N; i++)
  {
    ...
    {
      if (weigths[i - 1] <= w)
        dp[i][w] = max(dp[i - 1][w], values[i - 1] + dp[i - 1][w - weigths[i - 1]]);
      else
        dp[i][w] = dp[i - 1][w];
      deb(i, w);
      deb(weigths[i - 1]);
      deb(values[i - 1]);
      deb(dp[i - 1][w - weigths[i - 1]]);
      deb(dp[i - 1][w]);
      // debugMatrix();
      LINE;
    }
...
void solve()
{
  ...
  weigths.resize(N, 0);
  values.resize(N, 0);
  dp.resize(N + 1, vector<int>(MAXW + 1, 0));

  rep(i, 0, N)
  {
    cinn(values[i], weigths[i]);
  }
