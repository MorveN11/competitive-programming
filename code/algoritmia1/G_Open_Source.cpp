...
struct Project
{
  ...
  Project(const string &n, int a) : name(n), count(a) {}
};
...
void solve()
{
  while (true)
  {
    ...
    {
      projectList.push_back(Project(project.first, project.second));
    }

    sort(all(projectList), ProjectComparator());

    each(project, projectList)
    {
      cout << project.name << " " << project.count << endl;
    }
  }
}
